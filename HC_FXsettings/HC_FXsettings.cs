...
namespace BepInEx
{
    ...
    public class HC_FXsettings : BasePlugin
    {
        internal const string GUID = "HC_FXsettings";
        internal const string PluginName = "HC_FXsettings";
        ...
        public static Camera managerCamera;

        public override void Load()
        {
            AutoApply = Config.Bind("Apply on Startup (has to be enabled or effects will reset on map change)", "Apply on Startup", true, "Apply graphics settings when you start the game. May also force the resolution when the game is running");
            ...
            AllowMSAA.SettingChanged += (sender, args) => ApplyAntialiasingSettings();
            MSAAQuality = Config.Bind("Anti aliasing", "MSAA quality", 0, new ConfigDescription("Set MSAA quality", new AcceptableValueList<int>(0, 2, 4, 8)));
            MSAAQuality.SettingChanged += (sender, args) => ApplyAntialiasingSettings();
            ...
            SoftwareAntiAliasing.SettingChanged += (sender, args) => ApplyAntialiasingSettings();
            SoftwareAntiAliasingQuality = Config.Bind("Anti Aliasing", "Postprocess antialiasing quality", swAntiAliasingQuality.High, "Set postprocess antialiasing quality");
            SoftwareAntiAliasingQuality.SettingChanged += (sender, args) => ApplyAntialiasingSettings();
            ...
            {
                //Check if map gameobject has been loaded
                if (AutoApply.Value && (s.GetRootGameObjects()[0].name == "Map"))
                {
                    ApplySettings();
        ...
        {
            //Get volume profile and override settings
            volume = BeautifySettings.FindBeautifyVolume();
            volume.profile.TryGet(out Beautify.Universal.Beautify beautifyInstance);
        ...
        {
            //Get post process and fx camera and apply MSAA settings
            managerCamera = GameObject.Find("Manager(Clone)").GetComponentInChildren<Camera>();

            if (managerCamera != null)
            {
                managerCamera.allowMSAA = AllowMSAA.Value;
                QualitySettings.antiAliasing = MSAAQuality.Value;

                //Apply software antialiasing mode
                if (SoftwareAntiAliasing.Value == swAntiAliasingMode.None)
                {
                    managerCamera.GetUniversalAdditionalCameraData().antialiasing = AntialiasingMode.None;
                }
                else if (SoftwareAntiAliasing.Value == swAntiAliasingMode.SMAA)
                {
                    managerCamera.GetUniversalAdditionalCameraData().antialiasing = AntialiasingMode.SubpixelMorphologicalAntiAliasing;
                }
                else if (SoftwareAntiAliasing.Value == swAntiAliasingMode.FXAA)
                {
                    managerCamera.GetUniversalAdditionalCameraData().antialiasing = AntialiasingMode.FastApproximateAntialiasing;
                }
                //Apply software antialiasing quality
                if (SoftwareAntiAliasingQuality.Value == swAntiAliasingQuality.High)
                {
                    managerCamera.GetUniversalAdditionalCameraData().antialiasingQuality = AntialiasingQuality.High;
                }
                else if (SoftwareAntiAliasingQuality.Value == swAntiAliasingQuality.Medium)
                {
                    managerCamera.GetUniversalAdditionalCameraData().antialiasingQuality = AntialiasingQuality.Medium;
                }
                else if (SoftwareAntiAliasingQuality.Value == swAntiAliasingQuality.Low)
                {
                    managerCamera.GetUniversalAdditionalCameraData().antialiasingQuality = AntialiasingQuality.Low;
                }
            }
            else
            {
                Log.LogError("Manager camera not found");
            }
        }
    }
}
